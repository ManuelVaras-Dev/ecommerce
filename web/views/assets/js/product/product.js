  ...
  if(variant.offer_variant > 0){
  	...
  	$(".blockPrice").html(`
  		...
  		<h5 class="my-3 text-center font-weight-bold text-danger">¡Aprovecha la PROMO y llévalo a un increíble precio!<br>↓↓↓</h5>
			...
			<h3 class="text-center">ANTES 
				<s>S/${Number(variant.price_variant).toFixed(2)}</s>
			</h3>

			<h3 class="text-center">
				<span class="text-success pt-4">AHORA S/${Number(variant.offer_variant).toFixed(2)}</span>
				<span class="ml-2 px-2 p-1 small rounded-pill" 
				style="font-size: 16px; position:relative; top:-4px; border:2px solid #000 !important">
					AHORRE S/${(Number(variant.offer_variant)-Number(variant.price_variant)).toFixed(2)}			
				</span>
			</h3>
  	...
  	`)
  	 ...
  	 $(".blockPrice").html(`
        ...
        <h2 class="text-center"><span class="text-success pt-4">S/${Number(variant.price_variant).toFixed(2)}</span></h2>  
     `)
...
if($(".questionProduct").length > 0){
	...
	questionProduct.each((i)=>{
		...
		$(questionProduct[i]).attr("href","https://wa.me/"+$(questionProduct[i]).attr("phone")+"?text="+encodeURIComponent("¡Hola! me podria brindar información del producto ["+$(questionProduct[i]).attr("name")+"]"));
